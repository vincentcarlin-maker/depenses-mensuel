                  ...
                  <div className="bg-white dark:bg-slate-800 p-5 sm:p-7 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/80 space-y-5">
                    ...
                    <div className="flex gap-2.5 items-center">
                      ...
                      <button
                        ...
                        className={`relative w-11 h-11 shrink-0 rounded-2xl border flex items-center justify-center transition-all shadow-2xs ${
                          showFilters || filterUser !== 'All' || filterCategory !== 'All'
                            ...
                            : 'bg-slate-50/80 dark:bg-slate-700/50 border-slate-200/80 dark:border-slate-600/80 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700'
                        }`}
                        aria-label="Filtres"
                      >
                        <FunnelIcon />
                        {(filterUser !== 'All' || filterCategory !== 'All') && (
                          <span className="absolute top-2 right-2 w-2 h-2 rounded-full bg-blue-600 dark:bg-blue-400" />
                        )}
                      </button>
                    </div>

                    {showFilters && (
                      <div className="p-4 sm:p-5 bg-slate-50 dark:bg-slate-700/30 rounded-2xl border border-slate-200/80 dark:border-slate-700 space-y-4 animate-fade-in">
                        {/* Par membre */}
                        <div>
                          ...
                          <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
                            <button
                              ...
                              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs sm:text-sm font-bold transition-all border shrink-0 cursor-pointer ${
                                filterUser === 'All'
                                  ? 'bg-blue-100/90 dark:bg-blue-950/80 border-blue-200/90 dark:border-blue-800 text-blue-600 dark:text-blue-300 shadow-2xs'
                                  : 'bg-white dark:bg-slate-800 border-slate-200/80 dark:border-slate-600/80 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700'
                              }`}
                            >
                            ...
                            {(currentFoyer?.members || DEFAULT_FOYER.members).map((member, mIdx) => {
                              ...
                              if (isSelected) {
                                ...
                                pillClass = `bg-white dark:bg-slate-800 border-slate-200/80 dark:border-slate-600/80 ${theme.textClass} hover:opacity-90`;
                                iconColor = theme.textClass;
                              }
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs sm:text-sm font-bold transition-all border shrink-0 cursor-pointer ${pillClass}`}
                                >
                                  <UserIcon className={`w-3.5 h-3.5 ${iconColor}`} />
                            ...
                            <button
                              ...
                              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs sm:text-sm font-bold transition-all border shrink-0 cursor-pointer ${
                                filterUser === User.Commun
                                  ...
                                  : 'bg-white dark:bg-slate-800 border-slate-200/80 dark:border-slate-600/80 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-50 dark:hover:bg-slate-700'
                              }`}
                            >
