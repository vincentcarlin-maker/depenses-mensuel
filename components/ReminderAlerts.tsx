...
const ReminderAlerts: React.FC<ReminderAlertsProps> = ({ reminders, monthlyExpenses, onPayReminder, currentYear, currentMonth, loggedInUser }) => {
  ...
  const pendingReminders = reminders.filter(reminder => {
    ...
    const isPaid = monthlyExpenses.some(expense => {
        ...
        // 1. Vérification de la description (l'une contient l'autre)
        ...
        // Si la catégorie correspond EXACTEMENT et que le rappel est générique (ex: Rappel "Courses" vs Dépense "Leclerc") -> Payé
        // On considère un rappel comme générique si sa description est identique à sa catégorie.
